...
import { db, generateId, dbStatus, type CollectionSchema, type RecordEntry, type SavedFilter, type SavedView, type RecordTemplate, type FieldConfig } from '../db';
...
import { 
  ...
  X, Check, AlertCircle, LayoutDashboard, Sparkles, Bell, Copy
} from '@lucide/vue';
...
const setupDefaultSorting = () => {
  if (activeCollection.value) {
    if (activeCollection.value.defaultSortByField !== undefined) {
      sortByField.value = activeCollection.value.defaultSortByField;
      sortOrder.value = activeCollection.value.defaultSortOrder || 'desc';
    } else {
...
const handleRelationChange = async (field: FieldConfig, selectedId: string) => {
  if (!field.autoFillMappings || !selectedId) return;

  const relatedRecs = relatedRecordsCache.value[field.relatedCollectionId!] || [];
  const selectedRecord = relatedRecs.find(r => r.id === selectedId);

  if (!selectedRecord) return;

  field.autoFillMappings.forEach(mapping => {
    const sourceVal = selectedRecord.data[mapping.sourceKey];
    if (sourceVal !== undefined) {
      recordFormData.value[mapping.targetKey] = JSON.parse(JSON.stringify(sourceVal));
    }
  });
};
...
const openDuplicateModal = (rec: RecordEntry) => {
  if (!activeCollection.value) return;
  editingRecordId.value = null;
  pendingNotificationTrigger.value = null;
  // Clone data to form (pre-fill with source row values)
  recordFormData.value = JSON.parse(JSON.stringify(rec.data));
  tempTagInputs.value = {};
  showFormModal.value = true;
};
...
const handleSaveRecord = async () => {
  ...
  try {
    ...
    if (pendingNotificationTrigger.value) {
      ...
      await db.templates.update(template.id, { lastGeneratedPeriod: period, updatedAt: Date.now() });
      pendingNotificationTrigger.value = null;
...
const formatValue = (val: any, fieldConfig?: any) => {
  ...
  if (fieldConfig?.type === 'number') {
    ...
    if (!isNaN(num)) {
      ...
      } else if (fieldConfig.decimals !== undefined && fieldConfig.decimals !== null && fieldConfig.decimals !== '') {
...
const checkAutomatedTemplates = () => {
  ...
  for (const t of templates.value) {
    ...
    for (const cand of candidates) {
      if (t.lastGeneratedPeriod && t.lastGeneratedPeriod >= cand.period) continue;
...
const triggerAutomatedTemplate = (notif: TemplateNotification) => {
  ...
  activeNotification.value = null; // Hide instantly from the background
  editingRecordId.value = null;
  showFormModal.value = true;
};

const dismissNotification = async (notif: TemplateNotification) => {
  activeNotification.value = null; // Hide instantly from UI
  try {
    await db.templates.update(notif.template.id, { lastGeneratedPeriod: notif.period, updatedAt: Date.now() });
    await loadTemplates(); // Reload to chain the next notification immediately
  } catch (err) {
              ...
              <tr v-else v-for="rec in paginatedRecords" :key="rec.id">
                ...
                <td class="text-right actions-column">
                  <div class="cell-actions">
                    <button @click="openDuplicateModal(rec)" class="btn-action-round duplicate" title="Dupliquer">
        ...
        <div v-if="showFormModal" class="modal-overlay" @click.self="closeFormModal">
          <div class="modal-card card glass fade-in">
            <div class="modal-header">
              ...
              <button @click="closeFormModal" class="btn-close-modal"><X /></button>
            </div>
                ...
                <div v-for="f in activeCollection?.fields" :key="f.id" class="form-group">
                  ...
                  <div v-else-if="f.type === 'relation' && f.relatedCollectionId" class="relation-field-wrapper">
                    ...
                    <select
                      ...
                      :id="'form-' + f.key"
                      v-model="recordFormData[f.key]"
                      @change="handleRelationChange(f, recordFormData[f.key])"
                      class="form-control"
                      :required="f.required"
                      :disabled="isFieldDisabled(f)"
                    >
