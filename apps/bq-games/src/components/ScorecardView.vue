...
onMounted(() => {
  ...
  const savedSession = localStorage.getItem('bq-games-active-session');
  if (savedSession) {
    try {
...
watch(activeGameDetails, (newDetails) => {
  if (isActiveGame.value) {
    localStorage.setItem('bq-games-active-session', JSON.stringify(newDetails));
  }
}, { deep: true });
...
const startGame = () => {
  ...
  isActiveGame.value = true;
  localStorage.setItem('bq-games-active-session', JSON.stringify(activeGameDetails.value));
  initializeRoundInput();
};
...
const endAndSaveGame = async () => {
  ...
  try {
    ...
    localStorage.removeItem('bq-games-active-session');
