...
L'application utilise une base IndexedDB locale hors-ligne (`BQPlayDatabase`) et réutilise les identifiants de collections de `bq-metrics` :
...
* **Vérifier les types TypeScript & Compiler le projet (PWA)** :
  ```bash
  npm run build -w bq-play
  ```

* **Tester les assets compilés en local** :
  ```bash
  npm run preview -w bq-play
  ```
